...
/usr/local/share/slurm_gcp_docker/src/hung_disk_daemon.py &
...
while true; do
	...
	if [[ $RC == 124 || $RC == 0 ]]; then # 124 -> timeout; 0 -> grep succeeded
		timeout 1 bash -c "echo \"$HOSTNAME self-destructing due to nonresponsive controller!\" >> /mnt/nfs/clust_conf/.selfdestruct.log"
		gcloud compute instances delete $HOSTNAME --zone $ZONE --quiet
	fi
