        ...
        try:
            ...
            if mountpoint != "" and subprocess.run(f"flock -n {mountpoint} true", shell = True).returncode == 0:
                ...
                if disk in bad_disks:
                    ...
                    subprocess.check_call(f"CLOUDSDK_CONFIG=/slurm_gcloud_config gcloud_exp_backoff 320 compute instances detach-disk {HOSTNAME} --zone {ZONE} --disk {disk}", shell = True)
